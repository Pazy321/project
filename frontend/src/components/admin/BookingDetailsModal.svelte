  ...
  <div class="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
    <div class="bg-gradient-to-r from-primary-dark to-primary text-white p-6 flex justify-between items-center rounded-t-2xl">
      <h2 class="text-xl md:text-2xl flex items-center gap-2">
        Детали бронирования <span>#{booking.id}</span>
      </h2>
    ...
    <div class="p-6 md:p-8">
      <div class="booking-details">
        <div class="detail-section mb-6">
          <h3 class="text-lg font-bold text-gray-800 mb-4">Информация о бронировании</h3>
          ...
          <div class="detail-item flex justify-between py-2 border-b border-gray-200">
            <span class="detail-label text-gray-600">Создано:</span>
            <span class="detail-value">{formatDateTime(booking.created_at)}</span>
          </div>
          <div class="detail-item flex justify-between py-2 border-b border-gray-200">
            <span class="detail-label text-gray-600">Статус:</span>
        ...
        <div class="detail-section mb-6">
          <h3 class="text-lg font-bold text-gray-800 mb-4">Информация о госте</h3>
          <div class="detail-item flex justify-between py-2 border-b border-gray-200">
            <span class="detail-label text-gray-600">Имя:</span>
            <span class="detail-value">{booking.guest_name || 'Не указано'}</span>
          </div>
          <div class="detail-item flex justify-between py-2 border-b border-gray-200">
            <span class="detail-label text-gray-600">Телефон:</span>
            <span class="detail-value">{booking.guest_phone || 'Не указано'}</span>
          </div>
          <div class="detail-item flex justify-between py-2 border-b border-gray-200">
            <span class="detail-label text-gray-600">Email:</span>
            <span class="detail-value">{booking.guest_email || 'Не указано'}</span>
          </div>
        </div>
        
        <div class="detail-section mb-6">
          <h3 class="text-lg font-bold text-gray-800 mb-4">Даты проживания</h3>
          <div class="detail-item flex justify-between py-2 border-b border-gray-200">
            <span class="detail-label text-gray-600">Заезд:</span>
            <span class="detail-value"><strong>{formatDate(booking.checkin_date)}</strong></span>
          </div>
          <div class="detail-item flex justify-between py-2 border-b border-gray-200">
            <span class="detail-label text-gray-600">Выезд:</span>
            <span class="detail-value"><strong>{formatDate(booking.checkout_date)}</strong></span>
          </div>
          <div class="detail-item flex justify-between py-2 border-b border-gray-200">
            <span class="detail-label text-gray-600">Ночей:</span>
            <span class="detail-value">{nights}</span>
          </div>
        </div>
        
        <div class="detail-section mb-6">
          <h3 class="text-lg font-bold text-gray-800 mb-4">Гости</h3>
          <div class="detail-item flex justify-between py-2 border-b border-gray-200">
            <span class="detail-label text-gray-600">Взрослых:</span>
            <span class="detail-value">{booking.adults}</span>
          </div>
          <div class="detail-item flex justify-between py-2 border-b border-gray-200">
            <span class="detail-label text-gray-600">Детей:</span>
            <span class="detail-value">{booking.children}</span>
          </div>
          <div class="detail-item flex justify-between py-2 border-b border-gray-200">
            <span class="detail-label text-gray-600">Младенцев:</span>
            <span class="detail-value">{booking.infants}</span>
          </div>
        </div>
        
        <div class="detail-section">
          <h3 class="text-lg font-bold text-gray-800 mb-4">Цена</h3>
          <div class="detail-item flex justify-between py-2">
            <span class="detail-label text-gray-600">Итого:</span>
    ...
    <div class="p-5 md:p-6 bg-gray-50 rounded-b-2xl flex justify-end gap-3 md:gap-4 border-t border-gray-200">
      ...
      {#if booking.status === 'pending'}
        <button class="btn bg-red-500 text-white px-6 py-3 rounded-lg font-semibold hover:bg-red-600 transition-colors" on:click={handleCancel}>Отменить</button>
        <button class="btn-primary" on:click={handleConfirm}>Подтвердить</button>
